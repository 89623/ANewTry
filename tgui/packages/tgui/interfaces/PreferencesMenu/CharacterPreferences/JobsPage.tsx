...
function JobRow(props: JobRowProps) {
  ...
  const selectedAltTitleDisplay =
    job.localized_alt_titles?.[selectedAltTitle] ||
    (selectedAltTitle === name ? job.display_name || name : selectedAltTitle);
  const altTitleOptions = (job.alt_titles ?? []).map((altTitle) => ({
    displayText: job.localized_alt_titles?.[altTitle] || altTitle,
    value: altTitle,
  }));
          ...
          >
            {
              ...
              !job.alt_titles ? (
                ...
                <Dropdown
                  ...
                  displayText={selectedAltTitleDisplay}
                  onSelected={(value) =>
...
function JoblessRoleDropdown(props) {
  ...
  const overflowRoleDisplay =
    serverPrefs?.jobs.jobs[data.overflow_role]?.display_name || data.overflow_role;

  const options = [
    {
      displayText: `Join as ${overflowRoleDisplay} if unavailable`,
