  ...
  return (
    ...
    >
      <Stack fill>
        ...
        <Stack.Item grow>{job.command ? <b>{job.display_name || jobName}</b> : job.display_name || jobName}</Stack.Item>
        <Stack.Item>
          <span
            style={{
              whiteSpace: 'nowrap',
          ...
          {Object.entries(department.jobs).map(([name, job]) => (
            <Stack.Item key={name}>
              <JobEntry
                ...
                department={department}
                onClick={() => {
                  console.log('LATEJOIN DEBUG: click select_job', name);
...
export function JobSelection(props) {
  ...
  if (!data?.departments || !data?.departments_static) {
