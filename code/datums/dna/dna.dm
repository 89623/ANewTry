...
/datum/dna/proc/update_instability(alert=TRUE)
	...
	if(holder)
		...
		if(alert)
			switch(stability)
				if(70 to 90)
					message = span_warning("你打了个寒颤。")
				if(60 to 69)
					message = span_warning("你感到寒冷。")
				if(40 to 59)
					message = span_warning("你感到恶心。")
				if(20 to 39)
					message = span_warning("感觉就像你的皮肤在移动。")
				if(1 to 19)
					message = span_warning("你能感觉到你的细胞在燃烧。")
				if(-INFINITY to 0)
					message = span_boldwarning("你能感觉到你的DNA正在爆炸，我们必须尽快做点什么！")
		if(stability <= 0)
...
/mob/living/carbon/human/proc/something_horrible_mindmelt()
	if(!is_blind())
		...
		eyes.Remove(src)
		qdel(eyes)
		visible_message(span_notice("[src]抬起头，他们的眼睛融化了！"), span_userdanger("我现在明白了。"))
