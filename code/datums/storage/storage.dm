...
/datum/storage/proc/topic_handle(datum/source, user, href_list)
	...
	if(href_list["show_valid_pocket_items"])
		to_chat(user, span_notice("[source]可以容纳：[can_hold_description]"))
...
/datum/storage/proc/handle_examination(datum/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("你可以进一步检查它以了解它能容纳哪些额外物品。")
...
/datum/storage/proc/handle_extra_examination(datum/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("[source]可以容纳：[can_hold_description]")
...
/datum/storage/proc/can_insert(obj/item/to_insert, mob/user, messages = TRUE, force = STORAGE_NOT_LOCKED)
	...
	if(!can_hold_it || cant_hold_it || trait_says_no)
		if(messages && user)
			user.balloon_alert(user, "装不下！")
		return FALSE

	if(HAS_TRAIT(to_insert, TRAIT_NODROP))
		if(messages && user)
			user.balloon_alert(user, "粘在手上了！")
		return FALSE
	...
	if(bigger_fish && bigger_fish.max_specific_storage < max_specific_storage)
		if(messages && user)
			user.balloon_alert(user, "[LOWER_TEXT(parent.loc.name)]挡路了！")
		return FALSE
...
/datum/storage/proc/item_insertion_feedback(mob/user, obj/item/thing, override = FALSE)
	...
	if(!silent_for_user)
		to_chat(user, span_notice("你将[thing][insert_preposition]到[parent]。"))

	for(var/mob/viewing in oviewers(user))
		if(in_range(user, viewing) || (thing?.w_class >= WEIGHT_CLASS_NORMAL))
			viewing.show_message(span_notice("[user]将[thing][insert_preposition]到[parent]。"), MSG_VISUAL)
...
/datum/storage/proc/collect_on_turf(obj/item/thing, mob/user)
	...
	if(!amount)
		parent.balloon_alert(user, "没有东西可捡！")
		return
...
/datum/storage/proc/dump_content_at(atom/dest_object, dump_loc, mob/user)
	...
	if(dest_object.atom_storage)
		to_chat(user, span_notice("你将[parent]中的物品倒进了[dest_object]。"))
	...
	to_chat(user, span_notice("你开始将[parent]中的物品倒在[dest_object]上..."))
...
/datum/storage/proc/open_storage(mob/living/to_show, can_reach_target = parent) // NOVA EDIT CHANGE - ORIGINAL: /datum/storage/proc/open_storage(mob/living/to_show)
	...
	if(locked)
		if(!silent)
			parent.balloon_alert(to_show, "关上了！")
		return FALSE
		...
		if (remove_single(to_show, to_remove))
			...
			if(!silent)
				to_show.visible_message(
					span_warning("[to_show] 从 [to_remove] 中取出了 [parent]！"),
					span_notice("你从[to_remove]中取出了[parent]。"),
				)
			return TRUE
...
/datum/storage/proc/put_in_hands_async(mob/to_show, obj/item/toremove)
	if(!to_show.put_in_hands(toremove))
		if(!silent)
			toremove.balloon_alert(to_show, "失手了！")
		return TRUE
...
/datum/storage/proc/toggle_collection_mode(mob/user)
	...
	switch(collection_mode)
		if(COLLECT_SAME)
			parent.balloon_alert(user, "现在只会拾取单一类型物品")
		if(COLLECT_EVERYTHING)
			parent.balloon_alert(user, "现在会拾取所有物品")
		if(COLLECT_ONE)
			parent.balloon_alert(user, "现在会一次拾取一个物品")
...
/datum/storage/proc/contents_changed_w_class(datum/source, obj/item/changed, old_w_class, new_w_class)
	...
	changed.visible_message(span_warning("[changed] 从 [parent] 里掉出来了！"), vision_distance = COMBAT_MESSAGE_RANGE)
