...
/datum/wound/blunt/bone/moderate/try_handling(mob/living/user)
	...
	if(user.grab_state >= GRAB_AGGRESSIVE)
		user.visible_message(span_danger("[user]开始扭动并拉扯[victim]脱臼的[limb.plaintext_zone]！"), span_notice("你开始扭动并拉扯[victim]脱臼的[limb.plaintext_zone]..."), ignored_mobs=victim)
...
/datum/wound/blunt/bone/moderate/proc/chiropractice(mob/living/carbon/human/user)
	...
	if(prob(65))
		user.visible_message(span_danger("[user]将[victim]脱臼的[limb.plaintext_zone]猛地推回了原位！"), span_notice("你将[victim]脱臼的[limb.plaintext_zone]猛地推回了原位！"), ignored_mobs=victim)
...
/datum/wound/blunt/bone/moderate/treat(obj/item/tool, mob/user)
	...
	if(victim == user)
		...
		user.visible_message(span_danger("[user]开始[scanned ? "expertly" : ""]用[victim]复位[limb.plaintext_zone]的[tool]。"), span_notice("你开始用[victim][limb.plaintext_zone]复位[tool]的[scanned ? ", keeping the holo-image's indications in mind" : ""]..."))
	...
	if(victim == user)
		...
		victim.apply_damage(10, BRUTE, limb, wound_bonus = CANT_WOUND)
		user.visible_message(span_danger("[user]完成了[victim]的[limb.plaintext_zone]的复位！"), span_nicegreen("你完成了[victim]的[limb.plaintext_zone]的复位！"), ignored_mobs=victim)
...
/datum/wound/blunt/bone/proc/gel(obj/item/stack/medical/bone_gel/I, mob/user)
	...
	user.visible_message(span_danger("[user] 开始匆忙地将 [I] 涂抹到 [victim] 的 [limb.plaintext_zone] 上..."), span_warning("你开始匆忙地将 [I] 涂抹到 [user == victim ? "your" : "[victim]'s"] [limb.plaintext_zone] 上，无视警告标签..."))
	...
	victim.emote("scream")
	if(user != victim)
		user.visible_message(span_notice("[user] 完成了将 [I] 涂抹到 [victim] 的 [limb.plaintext_zone] 上，发出嘶嘶声！"), span_notice("你完成了将 [I] 涂抹到 [victim] 的 [limb.plaintext_zone] 上！"), ignored_mobs=victim)
...
/datum/wound/blunt/bone/proc/skelly_gel(obj/item/stack/medical/bone_gel/I, mob/user)
	...
	user.visible_message(span_danger("[user]开始将[I]涂抹到[victim]的[limb.plaintext_zone]上..."), span_warning("你开始将[I]涂抹到[user == victim ? "your" : "[victim]'s"] [limb.plaintext_zone]上..."))
	...
	I.use(1)
	if(user != victim)
		user.visible_message(span_notice("[user]完成了将[I]涂抹到[victim]的[limb.plaintext_zone]上，发出一阵嘶嘶声！"), span_notice("你完成了将[I]涂抹到[victim]的[limb.plaintext_zone]上！"), ignored_mobs=victim)
...
/datum/wound/blunt/bone/proc/tape(obj/item/stack/medical/wrap/sticky_tape/surgical/I, mob/user)
	...
	user.visible_message(span_danger("[user] begins applying [I] to [victim]'s' [limb.plaintext_zone]..."), span_warning("你开始将[I]涂抹到[user == victim ? "your" : "[victim]'s"] [limb.plaintext_zone]上..."))
	...
	I.use(1)
	if(user != victim)
		user.visible_message(span_notice("[user]完成了将[I]涂抹到[victim]的[limb.plaintext_zone]上，发出一阵嘶嘶声！"), span_notice("你完成了将[I]敷在[victim]的[limb.plaintext_zone]上！"), ignored_mobs=victim)
