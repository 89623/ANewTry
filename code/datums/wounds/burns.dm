...
/datum/wound/burn/flesh/proc/uv(obj/item/flashlight/pen/paramedic/I, mob/user)
	...
	user.visible_message(span_notice("[user] 用 [victim] 照射 [limb] 的 [I] 上的烧伤。"), span_notice("你用 [user == victim ? "your" : "[victim]'s"] 照射 [limb.plaintext_zone] [I] 上的烧伤。"), vision_distance=COMBAT_MESSAGE_RANGE)
