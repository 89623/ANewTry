...
/datum/wound/slash/flesh/proc/lick_wounds(mob/living/carbon/human/user)
	...
	user.visible_message(span_notice("[user]开始舔舐[victim]的[limb.plaintext_zone]上的伤口。"), span_notice("你开始舔舐[victim]的[limb.plaintext_zone]上的伤口..."), ignored_mobs=victim)
	...
	user.visible_message(span_notice("[user]舔舐着[victim]的[limb.plaintext_zone]上的伤口。"), span_notice("你舔舐了[victim]的[limb.plaintext_zone]上的一些伤口"), ignored_mobs=victim)
...
/datum/wound/slash/flesh/proc/las_cauterize(obj/item/gun/energy/laser/lasgun, mob/user)
	...
	user.visible_message(span_warning("[user]开始将[lasgun]直接对准[victim]的[limb.plaintext_zone]..."), span_userdanger("你开始将[lasgun]直接对准[user == victim ? "your" : "[victim]'s"]的[limb.plaintext_zone]..."))
...
/datum/wound/slash/flesh/proc/tool_cauterize(obj/item/I, mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_WOUND_SCANNED))
		treatment_delay *= 0.5
		user.visible_message(span_danger("[user]开始用[victim]熟练地烧灼[limb.plaintext_zone]的[I]..."), span_warning("你开始用[user == victim ? "your" : "[victim]'s"]烧灼[limb.plaintext_zone]的[I]，同时留意着全息影像的指示……"))
	else
		user.visible_message(span_danger("[user]开始用[victim]烧灼[limb.plaintext_zone]的[I]……"), span_warning("你开始用[user == victim ? "your" : "[victim]'s"]烧灼[limb.plaintext_zone]的[I]……"))
