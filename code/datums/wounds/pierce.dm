...
/datum/wound/pierce/bleed/proc/tool_cauterize(obj/item/I, mob/user)
	...
	if(HAS_TRAIT(src, TRAIT_WOUND_SCANNED))
		treatment_delay *= 0.5
		user.visible_message(span_danger("[user] begins expertly cauterizing [victim]'s [limb.plaintext_zone] with [I]..."), span_warning("你开始用[user == victim ? "your" : "[victim]'s"]烧灼[limb.plaintext_zone][I]，同时留意着全息影像的指示..."))
	else
		user.visible_message(span_danger("[user] 开始用 [victim] 烧灼 [limb.plaintext_zone] 的 [I]..."), span_warning("你开始用 [user == victim ? "your" : "[victim]'s"] 烧灼 [limb.plaintext_zone] [I]..."))
