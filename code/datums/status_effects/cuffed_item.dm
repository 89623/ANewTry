...
/datum/status_effect/cuffed_item/proc/on_examine_more(datum/source, mob/user, list/examine_list)
	...
	examine_list += span_warning("[cuffed.examine_title(user)] 被 [cuffs.examine_title(user)] 绑在了 [owner.p_their()] [owner.get_held_index_name(owner.get_held_index_of_item(cuffed))] 上")
...
/datum/status_effect/cuffed_item/proc/cuffed_reminder(obj/item/item, mob/user, list/examine_texts)
	...
	if(user == owner)
		examine_texts += span_notice("[item.p_Theyre()] 被 \a [cuffs] 用手铐铐在你身上。你可以 <a href='byond://?src=[REF(item)];remove_cuffs_item=1'>移除它们</a>。")
...
/datum/status_effect/cuffed_item/proc/try_remove_cuffs(mob/living/user)
	...
	owner.balloon_alert(user, "手铐已从物品上移除")
...
/atom/movable/screen/alert/status_effect/cuffed_item
	name = "被铐住的物品"
	desc = "你有一个物品被牢牢铐在手臂上。短时间内你大概不会不小心把它掉在什么地方了。"
