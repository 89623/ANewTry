...
/datum/material_container/proc/on_examine(datum/source, mob/user, list/examine_texts)
	...
	for(var/I in materials)
		...
		if(amt)
			examine_texts += span_notice("其中储存了 [amt] 张 [LOWER_TEXT(M.name)] 板材。")
	...
	while(items.len)
		...
		if(first_checks)
			...
			if(target_item.atom_storage?.locked)
				if(!(mat_container_flags & MATCONTAINER_SILENT))
					to_chat(user, span_warning("[target_item] 的存储空间已锁定"))
				return
			...
			if(!isstack(target_item) && !target_item.atom_storage)
				...
				if(!has_space(total_amount))
					if(!(mat_container_flags & MATCONTAINER_SILENT))
						to_chat(user, span_warning("[parent] 没有足够的空间容纳 [target_item]！"))
					return
		...
		if(isstack(target_item) && precise_insertion)
			...
			var/requested_amount = tgui_input_number(user, "你想插入多少？", "插入 [item_stack.singular_name]", item_stack.amount, item_stack.amount)
				...
				switch(text2num(status))
					if(MATERIAL_INSERT_ITEM_SUCCESS) //no problems full item was consumed
						if(isstack(item))
							...
							if (sheets > 1)
								to_chat(user, span_notice("[sheets] 个[stack.singular_name][plural_s(stack.singular_name)] 已被添加到 [parent]。"))
							else
								to_chat(user, span_notice("该 [stack.singular_name] 已被添加到 [parent]。"))
						else
							if (count > 1)
								to_chat(user, span_notice("[count] 个[item.name][plural_s(item.name)]，价值 [amount] 张sheet\s ，已被添加到 [parent]。"))
							else
								to_chat(user, span_notice("\The [item]，价值 [amount] 张sheet\s ，[item.p_were()] 添加到 [parent]。"))
					if(MATERIAL_INSERT_ITEM_NO_SPACE) //no space
						to_chat(user, span_warning("[parent] 没有空间容纳 \the [item]！"))
					if(MATERIAL_INSERT_ITEM_NO_MATS) //no materials inside these items
						if(isstack(item))
							var/obj/item/stack/stack = item
							to_chat(user, span_warning("该 [stack.singular_name] 所含的材料无法被 [parent] 接受！"))
						else
							to_chat(user, span_warning("\The [item][count > 1 ? "[plural_s(item.name)] have" : " [item.p_have()]"] 没有 [parent] 可以接受的材料！"))
					if(MATERIAL_INSERT_ITEM_FAILURE) //could be because the material type was not accepted or other stuff
						if (isstack(item))
							var/obj/item/stack/stack = item
							to_chat(user, span_warning("这个[stack.singular_name]被[parent]拒绝了！"))
						else
							to_chat(user, span_warning("\The [item][count > 1 ? "[plural_s(item.name)] were" : " [item.p_were()]"] 被 [parent] 拒绝了！"))
