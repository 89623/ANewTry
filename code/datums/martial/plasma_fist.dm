...
/datum/martial_art/plasma_fist/proc/Plasma(mob/living/attacker, mob/living/defender)
	...
	if(!hasclient)
		to_chat(attacker, span_warning("为你的</span>[span_notice("Apotheosis")]<span class='warning'>汲取这种等离子能量会给氏族带来耻辱！"))
		...
		to_chat(attacker, span_warning("你无法再为你的</span>[span_notice("Apotheosis")]<span class='warning'>充能了！"))
		new /obj/effect/temp_visual/plasma_soul(Dturf)//doesn't beam to you, so it just hangs around and poofs.

	else
		plasma_power += plasma_increment
		to_chat(attacker, span_nicegreen("能量提升！你的</span>[span_notice("Apotheosis")]<span class='nicegreen'>现在能量等级为[plasma_power]！"))
