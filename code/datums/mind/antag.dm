...
/datum/mind/proc/give_uplink(silent = FALSE, datum/antagonist/antag_datum)
	...
	if(uplink_spawn_location == UPLINK_RADIO && cant_speak)
		if(!silent)
			to_chat(traitor_mob, span_warning("你被判定为不符合获得无线电上行链路的资格。将提供标准上行链路作为替代。"))
	...
	if(isnull(uplink_loc))
		...
		if(!silent)
			to_chat(traitor_mob, span_boldnotice("你的辛迪加上行链路已巧妙地植入你体内，只需少量TC费用。只需触发上行链路即可访问。"))
...
/datum/mind/proc/enslave_mind_to_creator(mob/living/creator)
	...
	if(creator.is_antag())
		...
		to_chat(current, span_userdanger("尽管你创造者当前的效忠对象可能改变，你真正的主人始终是[creator.real_name]。如果他们的忠诚改变，你的也会随之改变。除非你创造者的身体被摧毁，否则这永远不会改变。"))
