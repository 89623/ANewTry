...
/obj/item/food/clothing
	name = "临时飞蛾服装零食"
	desc = "如果你正在读这段话，那就说明我搞砸了。这事儿是关于飞蛾啃食衣物的，而我之前不知道还有别的更好的解决办法，就只能新建一个“食物”对象来处理。 <---臭小子斯提克写下的。"
...
/obj/item/clothing/examine(mob/user)
	...
	if(get_armor().has_any_armor() || (flags_cover & (HEADCOVERSMOUTH|PEPPERPROOF)) || (clothing_flags & STOPSPRESSUREDAMAGE) || (visor_flags & STOPSPRESSUREDAMAGE))
		. += span_notice("OOC：点击<a href='byond://?src=[REF(src)];list_armor=1'>此处</a>查看其防护等级。") // NOVA EDIT CHANGE - ORIGINAL: += span_notice("It has a <a href='byond://?src=[REF(src)];list_armor=1'>tag</a> listing its protection classes.")
