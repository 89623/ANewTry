...
/datum/hallucination/blood_flow/start()
	...
	to_chat(hallucinator, span_warning("你的 [bleeding_bodypart.plaintext_zone] 喷出了一股鲜血！"))
...
/datum/hallucination/blood_flow/proc/by_god()
	...
	to_chat(hallucinator, span_warning("血流不止，但[bleeding_bodypart.plaintext_zone]似乎并不疼痛..."))
...
/datum/hallucination/blood_flow/proc/stop_bleeding()
	...
	if(!QDELETED(bleeding_bodypart))
		to_chat(hallucinator, span_warning("你的[bleeding_bodypart.plaintext_zone]停止了流血。"))
	if(!QDELETED(src))
		qdel(src)
