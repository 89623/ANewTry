...
/obj/machinery/microwave/examine(mob/user)
	. = ..()
	if(vampire_charging_capable)
		. += span_info("本型号配备Wave™：Nanotrasen独家技术。我们最新最伟大的Wave™技术，允许您的PDA通过微波频率无线充电！您可以将设备放入其中并选择充电模式进行Wave充电。")
		. += span_info("因为没有什么比一边给PDA充电一边加热剩菜更能体现'未来感'了。Nanotrasen Wave™ - 重新定义多任务处理。")
	...
	if(!in_range(user, src) && !issilicon(user) && !isobserver(user))
		...
		return
	if(operating)
		. += span_notice("\The [src] 正在运行。")
		return

	if(length(ingredients))
		if(issilicon(user))
			. += span_notice("\The [src] 摄像头显示：")
		else
	...
	if(!(machine_stat & (NOPOWER|BROKEN)))
		...
		span_notice("- 功率：<b>[efficiency * TIER_1_CELL_CHARGE_RATE]W</b>。")
...
/obj/machinery/microwave/wirecutter_act(mob/living/user, obj/item/tool)
	...
	user.visible_message(
		span_notice("[user]开始修理[src]的一部分。"),
		span_notice("你开始修理[src]的一部分..."),
	)
	...
	user.visible_message(
		span_notice("[user]修好了[src]的一部分。"),
		span_notice("你修好了[src]的一部分。"),
	)
...
/obj/machinery/microwave/item_interaction(mob/living/user, obj/item/item, list/modifiers)
	...
	if(istype(item, /obj/item/stock_parts/power_store/cell) && cell_powered)
		...
		if(!user.transferItemToLoc(item, src))
			...
			return ITEM_INTERACT_BLOCKING
		cell = item
		balloon_alert(user, "[swapped ? "swapped" : "inserted"] 电池")
		update_appearance()
		return ITEM_INTERACT_SUCCESS
	...
	if(item.w_class <= WEIGHT_CLASS_NORMAL && !user.combat_mode && isnull(item.atom_storage))
		...
		user.visible_message(span_notice("[user] 将 \a [item] 添加到 \the [src]。"), span_notice("你将 [item] 添加到 \the [src]。"))
		update_appearance()
		return ITEM_INTERACT_SUCCESS
...
/obj/machinery/microwave/proc/handle_dumping(mob/living/user, obj/item/tool)
	...
	if(loaded)
		open(autoclose = 0.6 SECONDS)
		to_chat(user, span_notice("你将 [loaded] 件物品放入 \the [src]。"))
		update_appearance()
...
/obj/machinery/microwave/click_alt(mob/user, list/modifiers)
	...
	balloon_alert(user, "设置为[vampire_charging_enabled ? "charge" : "cook"]")
	playsound(src, 'sound/machines/beep/twobeep_high.ogg', 50, FALSE)
	if(HAS_SILICON_ACCESS(user))
		visible_message(span_notice("[user] 将\the [src]设置为[vampire_charging_enabled ? "charge" : "cook"]。"), blind_message = span_notice("你听到\the [src]发出提示音！"))
...
/obj/machinery/microwave/proc/wzhzhzh()
	...
	visible_message(span_notice("\The [src] 启动了。"), null, span_hear("你听到微波炉的嗡嗡声。"))
	operating = TRUE
...
/obj/machinery/microwave/proc/spark()
	visible_message(span_warning("火花在[src]周围飞溅！"))
		...
		if(successful_shock) //We only want to give feedback once, regardless of how many mobs got shocked
			...
			for(var/mob/smeller in get_hearers_in_view(DEFAULT_MESSAGE_RANGE, src))
				if(HAS_TRAIT(smeller, TRAIT_ANOSMIA))
					cant_smell += smeller
			visible_message(span_danger("你闻到一股烧焦的气味从[src]传来！"), ignored_mobs = cant_smell)
...
/obj/machinery/microwave/proc/muck_finish()
	visible_message(span_warning("\The [src] 被污物覆盖了！"))
...
/obj/machinery/microwave/engineering
	name = "无线微波炉"
	desc = "为身处偏远之地、只想加热从高价自动售货机买来的酥皮咸味点心的勤劳技工准备。"
