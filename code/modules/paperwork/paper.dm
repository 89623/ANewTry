...
/obj/item/paper/verb/rename()
	...
	if(ishuman(usr))
		...
		if(HAS_TRAIT(H, TRAIT_CLUMSY) && prob(25))
			to_chat(H, span_warning("你被纸割伤了！啊！啊啊啊！"))
...
/obj/item/paper/suicide_act(mob/living/user)
	user.visible_message(span_suicide("[user] 用纸在[user.p_their()]手腕上划了个格子！看起来[user.p_theyre()]想玩数独自杀..."))
	return BRUTELOSS

/obj/item/paper/examine(mob/user)
	. = ..()
	. += span_notice("Alt-点击 [src] 将其折成纸飞机。")
	if(!in_range(user, src) && !isobserver(user))
		. += span_warning("你离得太远，读不到上面的内容！")
		return
...
/obj/item/paper/proc/make_plane(mob/living/user, plane_type = /obj/item/paperplane)
	loc.balloon_alert(user, "折成了一架纸飞机")
...
/obj/item/paper/attackby(obj/item/attacking_item, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if(writing_stats["interaction_mode"] == MODE_WRITING)
		...
		if(get_total_length() >= MAX_PAPER_LENGTH)
			to_chat(user, span_warning("这张纸已经写满了！"))
			return
	...
	if(writing_stats["interaction_mode"] == MODE_STAMPING)
		if(!user.can_read(src) || user.is_blind())
			...
			user.visible_message(span_notice("[user]闭着眼睛用\the [src]在[attacking_item]上盖了个章！"))
			to_chat(user, span_notice("你尽力用\the [src]在[attacking_item]上盖了个章！"))
			playsound(src, 'sound/items/handling/standard_stamp.ogg', 50, vary = TRUE)
		else
			to_chat(user, span_notice("你将印章悬在纸张上方准备盖章！"))
			ui_interact(user)
		return
...
/obj/item/paper/item_interaction_secondary(mob/living/user, obj/item/tool, list/modifiers)
	...
	user.visible_message(
		span_notice("[user] 看都没看就用 [tool] 快速盖在了 [src] 上。"),
		span_notice("你看都没看就用 [tool] 快速盖在了 [src] 上。"),
	)
	playsound(src, 'sound/items/handling/standard_stamp.ogg', 50, vary = TRUE)
	...
	switch(action)
		if("add_stamp")
			...
			if(!stamp_info || (stamp_info["interaction_mode"] != MODE_STAMPING))
				to_chat(src, span_warning("你不能用 [holding] 盖章！"))
				return TRUE
			...
			if (LAZYLEN(raw_stamp_data) >= MAX_PAPER_STAMPS)
				to_chat(usr, pick("你试图盖章但没盖中！", "没有其他地方可以盖章了！"))
				return TRUE

			add_stamp(stamp_class, stamp_x, stamp_y, stamp_rotation, stamp_icon_state, stamp_icon)
			user.visible_message(span_notice("[user]用\the [src]给[holding.name]盖章！"), span_notice("You stamp [src] with \the [holding.name]!"))
			playsound(src, 'sound/items/handling/standard_stamp.ogg', 50, vary = TRUE)
		...
		if("add_text")
			...
			if(this_input_length == 0)
				to_chat(user, pick("写作瓶颈又来了！", "你忘了写任何东西！"))
				return TRUE
