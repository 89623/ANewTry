...
/datum/admins/proc/create_ban(player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, severity, reason, list/roles_to_ban)
	...
	var/msg = "has created a [isnull(duration) ? "permanent" : "temporary [time_message]"] [applies_to_admins ? "管理员" : ""][is_server_ban ? "server ban" : "role ban from [roles_to_ban.len] roles"] for [target]."
	log_admin_private("[kn] [msg][is_server_ban ? "" : "角色：[roles_to_ban.Join(", ")]"] Reason: [reason]")
	...
	var/player_ban_notification = span_boldannounce("You have been [applies_to_admins ? "admin " : ""]banned by [usr.client.key] from [is_server_ban ? "服务器" : "角色：[roles_to_ban.Join(", ")]"].\nReason: [reason]</span><br>[span_danger("此封禁是[isnull(duration) ? "permanent." : "temporary, it will be removed in [time_message]."] 回合ID为[GLOB.round_id]。")]")
...
/datum/admins/proc/notify_all_banned_players(banned_player_ckey, banned_player_ip, banned_player_cid, banned_player_message, banned_other_message, kick_banned_players, applies_to_admins)
	...
	if(player_client)
		...
		to_chat(player_client, span_boldannounce("[banned_player_message]<br><span class='danger'>要申诉此封禁，请前往 [appeal_url]"), confidential = TRUE)
	...
	for(var/client/other_player_client in GLOB.clients - player_client)
		if(other_player_client.address == banned_player_ip || other_player_client.computer_id == banned_player_cid)
			...
			to_chat(other_player_client, span_boldannounce("[banned_other_message]<br><span class='danger'>要申诉此封禁，请前往 [appeal_url]"), confidential = TRUE)
