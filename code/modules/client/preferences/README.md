...
职业偏好和迟到加入菜单的汉化要遵守同一个原则：`title`、`department_name`、`alt_titles` 这些英文值继续作为逻辑键使用，只在 `ui_data` / `ui_static_data` 里额外提供 `display_name` / `localized_alt_titles` 供 TGUI 展示。

这样做是因为职业标题会被 `SSjob.get_job()`、job ban、岗位解锁、出生点匹配、latejoin 选择和 `JOB2ICON` 等系统反复当作查找键使用。直接把逻辑键翻译成中文会把这些链路打断。

后续维护时，只需要更新 `code/_globalvars/lists/department.dm` 里的集中映射，不要修改职业 datum 本身的英文键，也不要把 `job_spawn_title` 改成中文。如果需要新增一个可显示的职业别名，先确认它只是 UI 文案，再把它放进 `localized_job_titles` 和 `localized_job_title_list()` 的结果里。
