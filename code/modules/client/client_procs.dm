...
/client/New(TopicData)
	...
	if(alert_mob_dupe_login && !holder)
		...
		if (alert_admin_multikey)
			...
			message_admins(span_danger("<B>多开检测：</B></span><span class='notice'>[key_name_admin(src)] 的CID与IP与另一名玩家匹配，明显为多开行为。已警告其离开服务器，否则将面临封禁风险。"))
	...
	*/ // NOVA EDIT REMOVAL END
	...
	if(!client_is_in_db)
		...
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			...
			to_chat_immediate(src, span_notice("您好！我们已临时启用安全措施，目前禁止新玩家加入。<br>请稍后再试，如有疑问请联系Discord管理员。<br><br>要加入我们的社区，请查看我们的Discord！要获得Discord的完全访问权限，请阅读规则并在Discord服务器的\"Landing Zone\"分类下的#access-requests频道中提交请求，Discord链接：<a href='https://discord.gg/novasector'>https://discord.gg/novasector</a>"))
			var/list/connectiontopic_a = params2list(connectiontopic)
			var/list/panic_addr = CONFIG_GET(string/panic_server_address)
			if(panic_addr && !connectiontopic_a["redirect"])
				var/panic_name = CONFIG_GET(string/panic_server_name)
...
/client/verb/toggle_fullscreen()
	set name = "Toggle Fullscreen-切换全屏"
	set category = "OOC"
