...
/obj/item/gibtonite/proc/defuse(mob/defuser)
	...
	if(det_timer)
		deltimer(det_timer)
	defuser?.visible_message(span_notice("连锁反应停止了！……矿石的品质看起来降低了。"), span_notice("你阻止了链式反应。……矿石的品质看起来下降了。"))
	icon_state = "gibtonite"
...
/obj/item/coin/proc/manual_suicide(mob/living/user)
	...
	if (index == 2)//tails
		...
		user.visible_message(span_suicide("\the [src] 落在了 [coinflip] 上！[user] 继续活了下来！")) //Don't put it in your pocket. It's your lucky quarter.
...
/obj/item/coin/gold/debug/attack_self(mob/user)
	if(cooldown < world.time)
		...
		SSeconomy.fire()
		to_chat(user,"<span class='bounty'>[SSeconomy.inflation_value()] 是通货膨胀值。</span>")
	return TRUE//did the coin flip? useful for suicide_act
