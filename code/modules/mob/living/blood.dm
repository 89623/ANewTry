	...
	switch(modified_blood_volume)
		...
		if(BLOOD_VOLUME_RISKY to BLOOD_VOLUME_OKAY)
			if(SPT_PROB(5, seconds_per_tick))
				...
				if(prob(50))
					to_chat(src, span_bolddanger("你感到非常[word]。呼吸变得困难！"))
					losebreath += 1 * determined_mod
				else if(get_stamina_loss() < 40 * determined_mod)
					to_chat(src, span_bolddanger("你感到非常[word]。保持清醒变得困难！"))
		...
		if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_RISKY)
			if(SPT_PROB(5, seconds_per_tick))
				...
				if(prob(50))
					to_chat(src, span_userdanger("你感到极度[word]！呼吸变得非常困难！"))
					losebreath += 1.5 * determined_mod
				else if(get_stamina_loss() < 80 * determined_mod)
					to_chat(src, span_userdanger("你感到极度[word]！保持清醒变得非常困难！"))
		...
		if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
			if(SPT_PROB(7.5, seconds_per_tick))
				...
				to_chat(src, span_userdanger("你眼前一黑，晕厥了片刻！"))
				// NOVA EDIT ADDITION START
				if(ishemophage(src))
					src.visible_message(span_warning("[src]抽搐着瘫倒在地，黑色的血管开始沿着他们的身体向外搏动。"), ignored_mobs = src)
