...
/mob/living/simple_animal/bot/mulebot/Initialize(mapload)
	...
	suffix = null
	if(name == "\improper 货运机器人")
		name = "\improper 骡子机器人 [id]"
...
/mob/living/simple_animal/bot/mulebot/examine(mob/user)
	...
	if(bot_cover_flags & BOT_COVER_MAINTS_OPEN)
		if(cell)
			. += span_notice("[p_They()] [p_have()] \a [cell] 已安装。")
			. += span_info("你可以使用<b>撬棍</b>来移除它。")
		else
			. += span_notice("[p_They()] [p_have()] 一个空置的隔间，可以在此安装<b>电源电池</b>。")
	if(load) //observer check is so we don't show the name of the ghost that's sitting on it to prevent metagaming who's ded.
		. += span_notice("\A [isobserver(load) ? "ghostly figure" : load] 在 [p_their()] 装载平台上。")
...
/mob/living/simple_animal/bot/mulebot/crowbar_act(mob/living/user, obj/item/tool)
	if(!(bot_cover_flags & BOT_COVER_MAINTS_OPEN) || user.combat_mode)
		return
	if(!cell)
		to_chat(user, span_warning("[src] 没有电源电池！"))
		return ITEM_INTERACT_BLOCKING
	cell.add_fingerprint(user)
	user.visible_message(
		span_notice("[user] 用撬棍从[src]中撬出了[cell]。"),
		span_notice("你从[src]中撬出了[cell]。"),
	)
...
/mob/living/simple_animal/bot/mulebot/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/stock_parts/power_store/cell) && (bot_cover_flags & BOT_COVER_MAINTS_OPEN))
		if(cell)
			to_chat(user, span_warning("[src] 已经有一个电源电池了！"))
			return ITEM_INTERACT_BLOCKING
		...
		diag_hud_set_mulebotcell()
		user.visible_message(
			span_notice("[user] 将 \a [cell] 插入 [src]。"),
			span_notice("你将[cell]插入[src]。"),
		)
		return ITEM_INTERACT_SUCCESS
...
/mob/living/simple_animal/bot/mulebot/attackby(obj/item/attacking_item, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if(ismob(load) && prob(1 + attacking_item.force * 2))
		user.visible_message(
			span_danger("[user] 用\the [attacking_item]将[load]从[src]上打了下来！"),
			span_danger("你用\the [attacking_item]将[load]从[src]上打了下来！"),
		)
		unload(0)
		return TRUE
...
/mob/living/simple_animal/bot/mulebot/emag_act(mob/user, obj/item/card/emag/emag_card)
	...
	balloon_alert(user, "控制装置 [bot_cover_flags & BOT_COVER_LOCKED ? "locked" : "unlocked"]")
...
/mob/living/simple_animal/bot/mulebot/bullet_act(obj/projectile/proj)
	...
	if(. && !QDELETED(src)) //Got hit and not blown up yet.
		if(prob(50) && !isnull(load))
			unload(0)
		if(prob(25))
			visible_message(span_danger("[src]内部有什么东西短路了！"))
	...
	switch(action)
		...
		if("on")
			if(bot_mode_flags & BOT_MODE_ON)
				...
				to_chat(user, span_warning("[name]的维护区面板是开着的！"))
				return
			else if(cell)
				if(!turn_on())
					to_chat(user, span_warning("你无法启动[src]！"))
					return
			return TRUE
		else
...
/mob/living/simple_animal/bot/mulebot/bot_control(command, mob/user, list/params = list(), pda = FALSE)
	...
	switch(command)
		...
		if("destination")
			var/new_dest
			if(pda)
				new_dest = tgui_input_list(user, "输入目的地", "骡子机器人设置", GLOB.deliverybeacontags, destination)
			else
				new_dest = params["value"]
			if(new_dest)
				set_destination(new_dest)
		if("setid")
			var/new_id = tgui_input_text(user, "输入ID", "ID分配", id, max_length = MAX_NAME_LEN)
			if(new_id)
				set_id(new_id)
				name = "\improper MULEbot [new_id]"
		if("sethome")
			var/new_home = tgui_input_list(user, "输入主站", "骡子机器人设置", GLOB.deliverybeacontags, home_destination)
			if(new_home)
...
/mob/living/simple_animal/bot/mulebot/proc/buzz(type)
	switch(type)
		if(SIGH)
			audible_message(span_hear("[src]发出一声叹息般的嗡嗡声。"))
			playsound(src, 'sound/machines/buzz/buzz-sigh.ogg', 50, FALSE)
		if(ANNOYED)
			audible_message(span_hear("[src]发出一阵恼人的嗡嗡声。"))
			playsound(src, 'sound/machines/buzz/buzz-two.ogg', 50, FALSE)
		if(DELIGHT)
			audible_message(span_hear("[src]发出一声欢快的叮当声！"))
			playsound(src, 'sound/machines/ping.ogg', 50, FALSE)
		if(CHIME)
			audible_message(span_hear("[src]发出一阵钟鸣般的声音！"))
...
/mob/living/simple_animal/bot/mulebot/proc/at_target()
	if(!reached_target)
		...
		if(pathset) //The AI called us here, so notify it of our arrival.
			...
			if(calling_ai)
				to_chat(calling_ai, span_notice("[icon2html(src, calling_ai)] [src]无线播放了一段钟鸣声！"))
...
/mob/living/simple_animal/bot/mulebot/MobBump(mob/M) // called when the bot bumps into a mob
	...
	if(wires.is_cut(WIRE_AVOIDANCE)) // usually just bumps, but if the avoidance wire is cut, knocks them over.
		if(iscyborg(L))
			visible_message(span_danger("[src]撞上了[L]！"))
		else if(L.Knockdown(8 SECONDS))
			log_combat(src, L, "knocked down")
			visible_message(span_danger("[src]撞倒了[L]！"))
	return ..()
...
/mob/living/simple_animal/bot/mulebot/proc/run_over(mob/living/carbon/human/crushed)
	if (!(bot_cover_flags & BOT_COVER_EMAGGED) && !wires.is_cut(WIRE_AVOIDANCE))
		if (!has_status_effect(/datum/status_effect/careful_driving))
			crushed.visible_message(span_notice("[src]减速以避免压到[crushed]。"))
	...
	crushed.visible_message(
		span_danger("[src]从[crushed]身上碾了过去！"),
...
/mob/living/simple_animal/bot/mulebot/paranormal/load(atom/movable/movable_atom)
	...
	if(isobserver(movable_atom))
		visible_message(span_warning("一个幽灵般的身影出现在[src]上！"))
...
/mob/living/simple_animal/bot/mulebot/paranormal/proc/ghostmoved()
	SIGNAL_HANDLER
	visible_message(span_notice("幽灵般的身影消失了……"))
