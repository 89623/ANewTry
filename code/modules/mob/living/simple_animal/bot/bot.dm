...
/mob/living/simple_animal/bot/proc/rename(mob/user)
	var/new_name = sanitize_name(
		reject_bad_text(tgui_input_text(
			user = user,
			message = "这台机器的指定名称为[real_name]。你想要更新它的注册信息吗？",
			title = "名称更改",
			default = real_name,
...
/mob/living/simple_animal/bot/examine(mob/user)
	...
	. += span_notice("[p_Their()] 的维护区面板是 [bot_cover_flags & BOT_COVER_MAINTS_OPEN ? "open" : "closed"]。")
	. += span_info("你可以使用 <b>螺丝刀</b> 来 [bot_cover_flags & BOT_COVER_MAINTS_OPEN ? "close" : "open"] 它。")
	if(bot_cover_flags & BOT_COVER_MAINTS_OPEN)
		. += span_notice("[p_Their()] 的控制面板是 [bot_cover_flags & BOT_COVER_LOCKED ? "locked" : "unlocked"]。")
		var/is_sillycone = HAS_SILICON_ACCESS(user)
		if(!(bot_cover_flags & BOT_COVER_EMAGGED) && (is_sillycone || user.Adjacent(src)))
			. += span_info("Alt-点击 [is_sillycone ? "" : "or use your ID on "] 来 [bot_cover_flags & BOT_COVER_LOCKED ? "un" : ""]锁 [p_their()] 的控制面板。")
	if(paicard)
...
/mob/living/simple_animal/bot/proc/unlock_with_id(mob/user)
	if(bot_cover_flags & BOT_COVER_EMAGGED)
		...
		return
	if(bot_cover_flags & BOT_COVER_MAINTS_OPEN)
		to_chat(user, span_warning("请在[bot_cover_flags & BOT_COVER_LOCKED ? "un" : ""]锁前关闭访问面板。"))
		return
	if(!allowed(user))
		to_chat(user, span_warning("Access denied."))
		return
	bot_cover_flags ^= BOT_COVER_LOCKED
	to_chat(user, span_notice("控制面板现已[bot_cover_flags & BOT_COVER_LOCKED ? "locked" : "unlocked"]。"))
	return TRUE
...
/mob/living/simple_animal/bot/screwdriver_act(mob/living/user, obj/item/tool)
	...
	to_chat(user, span_notice("维护面板现已[bot_cover_flags & BOT_COVER_MAINTS_OPEN ? "opened" : "closed"]。"))
	return ITEM_INTERACT_SUCCESS
...
/mob/living/simple_animal/bot/proc/call_bot(summoner, turf/waypoint, message = TRUE)
	...
	if(path?.len) //Ensures that a valid path is calculated!
		...
		if(client)
			...
			to_chat(src, span_notice("[span_big("Priority waypoint set by [icon2html(calling_ai, src)] <b>[summoner]</b>. Proceed to <b>[end_area]</b>.")]<br>[path.len-1] 米到达目的地。你已被授予额外的门禁权限，持续 60 秒。"))
		if(message)
...
/mob/living/simple_animal/bot/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("hack")
			...
			to_chat(user, span_notice("你重置了 [src] 的 [hackables]。"))
