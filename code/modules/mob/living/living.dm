...
/mob/living/proc/MobBump(mob/M)
	...
	if(isliving(M))
		...
		if(L.gunpointing)
			if(!(world.time % 5))
				to_chat(src, "<span class='warning'>[L]正用枪指着某人，你无法挤过去。</span>")
			return TRUE
		//NOVA EDIT ADDITION END
...
/mob/living/proc/process_capture(ransom_price, black_market_price)
	if(ransom_price > 0)
		...
		priority_announce("你方的一名船员被敌对组织捕获——我们不得不支付赎金将其带回。根据政策，我们已从空间站资金中抽取一部分以抵消总体成本。", "纳米传讯资产保护部", has_important_message = TRUE)
...
/mob/living/proc/admin_give_guardian(mob/admin)
	...
	if(isnull(guardian_client))
		...
		var/mob/chosen_one = SSpolling.poll_ghost_candidates("你愿意扮演管理员为[span_notice("Guardian Spirit")]创建的[span_danger(real_name)]吗？", check_jobban = ROLE_PAI, poll_time = 10 SECONDS, ignore_category = POLL_IGNORE_HOLOPARASITE, alert_pic = mutable_appearance('icons/mob/nonhuman-player/guardian.dmi', "magicexample"), jump_target = src, role_name_text = "守护灵", amount_to_pick = 1)
		if(chosen_one)
