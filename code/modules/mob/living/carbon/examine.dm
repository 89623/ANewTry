...
/mob/living/carbon/examine(mob/user)
	...
	if (!(face_obscured))
		flavor_text_link = span_notice("[preview_text]... <a href='byond://?src=[REF(src)];lookup_info=open_examine_panel'>\[要仔细看看吗？\]</a>")
	else
		flavor_text_link = span_notice("<a href='byond://?src=[REF(src)];lookup_info=open_examine_panel'>\[仔细检查...\]</a>")
	if (flavor_text_link)
		. += flavor_text_link
	if (!face_obscured && !HAS_TRAIT(src, TRAIT_UNKNOWN_APPEARANCE) && client?.prefs.read_preference(/datum/preference/text/character_ad))
		. += span_notice("[t_He] [t_has] 在角色目录里有一则广告... <a href='byond://?src=[REF(src)];lookup_info=open_character_ad'>\[打开目录？\]</a>")

	//Temporary flavor text addition:
	if(temporary_flavor_text)
		if(length_char(temporary_flavor_text) < TEMPORARY_FLAVOR_PREVIEW_LIMIT)
			. += span_revennotice("<br>他们看起来与往常不同：[temporary_flavor_text]")
		else
	...
	if (!CONFIG_GET(flag/disable_antag_opt_in_preferences))
		...
		if (!isnull(opt_in_status))
			...
			. += span_info("反派角色参与状态：<b><font color='[GLOB.antag_opt_in_colors[stringified_optin]]'>[stringified_optin]</font></b>")
...
/mob/living/carbon/proc/generate_death_examine_text()
	...
	if((brain && HAS_TRAIT(brain, TRAIT_GHOSTROLE_ON_REVIVE)) || HAS_TRAIT(src, TRAIT_GHOSTROLE_ON_REVIVE))
		return span_deadsay("[t_He] [t_is] 瘫软且毫无反应；但[t_his]灵魂或许还能回来...")
