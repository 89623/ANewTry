...
/mob/dead/new_player/proc/AttemptLateSpawn(rank)
	log_game("LATEJOIN DEBUG: [key_name(src)] start rank=[rank]")
	...
	log_game("LATEJOIN DEBUG: [key_name(src)] availability rank=[rank] error=[error]")
	...
	var/datum/job/job = SSjob.get_job(rank)
	log_game("LATEJOIN DEBUG: [key_name(src)] get_job rank=[rank] job=[job ? job.type : "null"] title=[job ? job.title : "null"]")
	...
	log_game("LATEJOIN DEBUG: [key_name(src)] spawn_point rank=[rank] destination=[destination ? "[destination.type] at [AREACOORD(destination)]" : "null"]")
	if(!destination)
		CRASH("Failed to find a latejoin spawn point.")
	var/mob/living/character = create_character(destination)
	log_game("LATEJOIN DEBUG: [key_name(src)] create_character rank=[rank] character=[character ? character.type : "null"]")
	if(!character)
		CRASH("Failed to create a character for latejoin.")
	transfer_character()
	log_game("LATEJOIN DEBUG: [key_name(character)] transfer_character done rank=[rank]")

	SSjob.equip_rank(character, job, character.client)
	log_game("LATEJOIN DEBUG: [key_name(character)] equip_rank done rank=[rank]")
	job.after_latejoin_spawn(character)
	log_game("LATEJOIN DEBUG: [key_name(character)] after_latejoin_spawn done rank=[rank]")
