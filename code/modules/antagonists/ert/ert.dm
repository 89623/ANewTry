...
/datum/antagonist/ert/official/greet()
	. = ..()
	if (ert_team)
		to_chat(owner, "<span class='warningplain'>中央指挥部正派遣你前往 [station_name()]，任务为：[ert_team.mission.explanation_text]</span>")
	else
		to_chat(owner, "<span class='warningplain'>中央指挥部正派遣你前往 [station_name()]，任务为：[mission.explanation_text]</span>")
