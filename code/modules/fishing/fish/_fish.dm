...
/obj/item/fish/suicide_act(mob/living/user)
	if(force == 0)
		user.visible_message(span_suicide("[user] 用 [src] 拍打 [user.p_them()] 自己，但什么都没发生！"))
		return SHAME
	user.visible_message(span_suicide("[user] 开始用 [src] 快速拍打 [user.p_them()] 自己！看起来 [user.p_theyre()] 试图自杀！"))
...
/obj/item/fish/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	user.visible_message(span_notice("[user] 用 [tool] 检查 [src] 的脉搏。"), span_notice("你用 [tool] 检查 [src] 的脉搏。"))
	var/warns = get_health_warnings(user, always_deep = TRUE)
	if(!warns)
		to_chat(user, span_notice("[src] 看起来非常健康！"))
...
/obj/item/fish/interact_with_atom_secondary(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(HAS_TRAIT(src, TRAIT_NODROP))
		balloon_alert(user, "[p_theyre()] 粘在你手上了！")
		return ITEM_INTERACT_BLOCKING
	balloon_alert(user, "放生鱼...")
	if(!do_after(user, 3 SECONDS, interacting_with))
		return ITEM_INTERACT_BLOCKING
	balloon_alert(user, "鱼已放生")
	var/goodbye_text = "Bye bye [name]."
	if(status == FISH_DEAD && !HAS_MIND_TRAIT(user, TRAIT_NAIVE))
		goodbye_text = "May [p_they()] rest in peace..."
	user.visible_message(span_notice("[user]将[src]放入了[interacting_with]"), \
		span_notice("你将[src]放入了[interacting_with]。[goodbye_text]"), \
		span_notice("你听到一阵水花声。"))
...
/obj/item/fish/proc/flinch_on_eat(mob/living/eater, mob/living/feeder)
	if(status == FISH_ALIVE && prob(50) && feeder.is_holding(src) && feeder.dropItemToGround(src))
		to_chat(feeder, span_warning("[src]因疼痛从你手中滑落！"))
...
/obj/item/fish/attackby(obj/item/item, mob/living/user, list/modifiers, list/attack_modifiers)
	...
	if(!item.reagents.total_volume)
		balloon_alert(user, "[item.name] 是空的！")
		return TRUE
	if(status == FISH_DEAD)
		balloon_alert(user, "[name] [HAS_MIND_TRAIT(user, TRAIT_NAIVE) ? "isn't hungry" : "is dead!"]")
		return TRUE
	feed(item.reagents)
	balloon_alert(user, "喂了 [name]")
	return TRUE

/obj/item/fish/examine(mob/user)
	. = ..()
	if(catcher_name && catch_date)
		. += span_boldnicegreen("由 [catcher_name] 于 [catch_date] 捕获。")

	if(HAS_MIND_TRAIT(user, TRAIT_EXAMINE_FISH) || HAS_TRAIT(loc, TRAIT_EXAMINE_FISH))
		. += span_notice("[p_theyre(TRUE)] [size] 厘米长。")
		. += span_notice("[p_they(TRUE)] 重 [weight] [span_tooltip("the standard unit of measurement for space age fish", "kiloclam")]。")

		if(HAS_TRAIT(src, TRAIT_FISH_GENEGUNNED))
			. += span_warning("[p_theyve(TRUE)] 已被鱼类基因枪编辑过。[p_they(TRUE)] 如果再次编辑就会死亡。")

	. += get_health_warnings(user, always_deep = FALSE)

	if(HAS_TRAIT(src, TRAIT_FISHING_BAIT))
		. += span_smallnoticeital("[p_they(TRUE)] 可以用作钓鱼饵料。")

	if(bites_amount)
		. += span_warning("[p_theyve(TRUE)] 被某人咬过。")
...
/obj/item/fish/process(seconds_per_tick)
	...
	if(time_passed_on_safe_turf >= (get_starvation_mult() ? STARVING_FISH_SUBMERGING_THRESHOLD : FISH_SUBMERGING_THRESHOLD))
		visible_message(span_notice("[src] 消散在\the [loc]中"), span_notice("你听到一阵水花声。"))
		released(loc)
...
/obj/item/fish/proc/use_lazarus(datum/source, obj/item/lazarus_injector/injector, mob/user)
	...
	if(injector.revive_type != SENTIENCE_ORGANIC)
		...
		return
	if(status != FISH_DEAD)
		balloon_alert(user, "[p_theyre(TRUE)] 还没死！")
		return
	set_status(FISH_ALIVE)
...
/obj/item/fish/proc/try_pet_fish(mob/living/user)
	...
	if(status == FISH_DEAD)
		to_chat(user, span_warning("你试图抚摸[src]，但[p_theyre()]一动不动！"))
		return FALSE
	if(!proper_environment())
		to_chat(user, span_warning("你试图抚摸[src]，但[p_theyre()]感觉不太舒服！"))
		return FALSE
...
/obj/item/fish/proc/pet_fish(mob/living/user, in_aquarium)
	if(fish_flags & FISH_FLAG_PETTED)
		if(in_aquarium)
			to_chat(user, span_warning("当你把手指伸进水里时，[src]从你的指尖游开了！"))
		else
			to_chat(user, span_warning("你试图抚摸[src]，但[p_they()]扭动着躲开了！"))
		return FALSE
	...
	if((/datum/fish_trait/predator in fish_traits) && prob(50))
		if(in_aquarium)
			user.visible_message(
				span_warning("[src]游来游去，然后咬了[user]一口！"),
				span_warning("[src]游来游去，然后咬了你一口！"),
				vision_distance = DEFAULT_MESSAGE_RANGE - 3,
			)
		else
			user.visible_message(
				span_warning("[src]咬了[user]的手！"),
				span_warning("你抱着[p_they()]抚摸[src]，结果[p_them()]开心地反咬了一口！"),
				vision_distance = DEFAULT_MESSAGE_RANGE - 3,
			)
		...
		if(in_aquarium)
			to_chat(user, span_notice("[src]游来游去！"))
		else
			to_chat(user, span_notice("你抱着[p_they()]抚摸[src]。"))
