...
/obj/item/organ/liver/examine(mob/user)
	...
	if(HAS_MIND_TRAIT(user, TRAIT_ENTRAILS_READER) || isobserver(user))
		if(HAS_TRAIT(src, TRAIT_LAW_ENFORCEMENT_METABOLISM))
			. += span_info("脂肪沉积和糖屑残留，暗示这是<em>安保</em>人员的肝脏。")
		if(HAS_TRAIT(src, TRAIT_CULINARY_METABOLISM))
			. += span_info("高铁含量和轻微的大蒜气味，暗示这是<em>厨师</em>的肝脏。")
		...
		if(HAS_TRAIT(src, TRAIT_COMEDY_METABOLISM))
			. += span_info("香蕉气味、滑腻光泽和按压时[span_clown("honking")]，暗示这是<em>小丑</em>的肝脏。")
		if(HAS_TRAIT(src, TRAIT_MEDICAL_METABOLISM))
			. += span_info("压力痕迹和淡淡的医用酒精味，暗示这是<em>医疗工作者</em>的肝脏。")
		if(HAS_TRAIT(src, TRAIT_ENGINEER_METABOLISM))
			. += span_info("有辐射暴露和太空适应迹象，暗示这是<em>工程师</em>的肝脏。")
		if(HAS_TRAIT(src, TRAIT_SCIENTIST_LIVER))
			. += span_info("奇怪的发光残留物、凝结的固态等离子体碎屑，以及看似肿瘤的东西表明，这是一个受过辐射的<em>科学家</em>的肝脏。")
		if(HAS_TRAIT(src, TRAIT_MAINTENANCE_METABOLISM))
			. += span_info("半消化的老鼠尾巴（不知怎么的）、恶心的污泥，以及微弱的灰牛气味暗示，这是一个<em>助手</em>肝脏的残骸。")
		if(HAS_TRAIT(src, TRAIT_CORONER_METABOLISM))
			. += span_info("泡菜和海水的香气，加上保存得异常完好，暗示这是一个<em>验尸官</em>肝脏的残骸。")
		if(HAS_TRAIT(src, TRAIT_HUMAN_AI_METABOLISM))
			. += span_info("这个肝脏看起来几乎不像人类且完全自给自足，暗示这是一个<em>人类AI</em>肝脏的残骸。")

		// royal trumps pretender royal
		if(HAS_TRAIT(src, TRAIT_ROYAL_METABOLISM))
			. += span_info("奢侈食物的丰富饮食、柔软床铺带来的柔韧性，暗示这是<em>部门主管</em>的肝脏。")
		else if(HAS_TRAIT(src, TRAIT_PRETENDER_ROYAL_METABOLISM))
			. += span_info("人造鱼子酱的饮食，以及失眠的迹象，暗示这是<em>想成为部门主管的人</em>的肝脏。")
...
/obj/item/organ/liver/organ_failure(seconds_per_tick)
	switch(failure_time/LIVER_FAILURE_STAGE_SECONDS)
		if(1)
			to_chat(owner, span_userdanger("你感到腹部一阵刺痛！"))
		if(2)
			to_chat(owner, span_userdanger("你感到肠道有灼烧感！"))
			owner.vomit(VOMIT_CATEGORY_DEFAULT)
		if(3)
			to_chat(owner, span_userdanger("你感到喉咙里有痛苦的胃酸！"))
			owner.vomit(VOMIT_CATEGORY_BLOOD)
		if(4)
			to_chat(owner, span_userdanger("剧烈的疼痛使你昏了过去！"))
			owner.vomit(VOMIT_CATEGORY_BLOOD, distance = rand(1,2))
			owner.emote("Scream")
			owner.AdjustUnconscious(2.5 SECONDS)
		if(5)
			to_chat(owner, span_userdanger("你感觉你的内脏快要融化了！"))
...
/obj/item/organ/liver/proc/on_owner_examine(datum/source, mob/user, list/examine_list)
	...
	switch(failure_time)
		if(0 to 3 * LIVER_FAILURE_STAGE_SECONDS - 1)
			examine_list += span_notice("[owner]的眼睛有些发黄。")
		if(3 * LIVER_FAILURE_STAGE_SECONDS to 4 * LIVER_FAILURE_STAGE_SECONDS - 1)
			examine_list += span_notice("[owner]的眼睛完全变黄了，而且[owner.p_they()]明显[owner.p_are()]在遭受痛苦。")
		if(4 * LIVER_FAILURE_STAGE_SECONDS to INFINITY)
			examine_list += span_danger("[owner]的眼睛完全变黄并且肿胀流脓。[owner.p_They()]看起来[owner.p_do()]像[owner.p_they()]还能活多久了。")
...
/obj/item/organ/liver/feel_for_damage(self_aware)
	...
	if(damage < high_threshold)
		return span_warning("你的[self_aware ? "liver" : "lower abdomen"]感到酸痛。")
	return span_boldwarning("你的[self_aware ? "liver" : "lower abdomen"]感觉像着火了一样！")
