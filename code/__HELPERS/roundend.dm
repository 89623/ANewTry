...
/datum/controller/subsystem/ticker/proc/give_show_report_button(client/C)
	...
	R.Grant(C.mob)
	to_chat(C,span_infoplain("<a href='byond://?src=[REF(R)];report=1'>再次显示回合结束报告</a>"))
...
/datum/controller/subsystem/ticker/proc/cheevo_report()
	...
	if(length(GLOB.achievements_unlocked))
		...
		for(var/datum/achievement_report/cheevo_report in GLOB.achievements_unlocked)
			parts += "<b>[cheevo_report.winner]</b> 在 [span_greentext("'[cheevo_report.cheevo]'")] 获得了 [cheevo_report.award_location] 成就！<BR>" // NOVA EDIT - No ckeys in the round end report - ORIGINAL: parts += "<BR>[cheevo_report.winner_key] was <b>[cheevo_report.winner]</b>, who earned the [span_greentext("'[cheevo_report.cheevo]'")] achievement at [cheevo_report.award_location]!<BR>"
		parts += "</ul>"
