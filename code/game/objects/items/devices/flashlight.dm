...
/obj/item/flashlight/glowstick/suicide_act(mob/living/carbon/human/user)
	if(!get_fuel())
		user.visible_message(span_suicide("[user] 试图将[src]的液体挤进[user.p_their()]眼睛里……但它是空的！"))
		return SHAME
	var/obj/item/organ/eyes/eyes = user.get_organ_slot(ORGAN_SLOT_EYES)
	if(!eyes)
		user.visible_message(span_suicide("[user] 试图将[src]的液体挤进[user.p_their()]眼睛里……但[user.p_they()]根本没有！"))
		return SHAME
