...
/obj/structure/window/examine(mob/user)
	...
	switch(state)
		if(WINDOW_SCREWED_TO_FRAME)
			. += span_notice("窗户已用<b>螺丝</b>固定在框架上。")
		if(WINDOW_IN_FRAME)
			. += span_notice("窗户的<i>螺丝已拧松</i>，但被<b>撬</b>进了框架。")
		if(WINDOW_OUT_OF_FRAME)
			if (anchored)
				. += span_notice("窗户已用<b>螺丝</b>固定在地板上。")
			else
				. += span_notice("窗户的<i>螺丝已从地板上拧松</i>，可以通过<b>扳手</b>进行解构。")
...
/obj/structure/window/attack_tk(mob/user)
	...
	user.visible_message(span_notice("有什么东西敲了敲 [src]。"))
...
/obj/structure/window/attack_hand(mob/living/user, list/modifiers)
	...
	if(!user.combat_mode)
		...
		user.visible_message(span_warning("[user] 猛击了 [src]！"), \
...
/obj/structure/window/screwdriver_act(mob/living/user, obj/item/tool)
	...
	switch(state)
		if(WINDOW_SCREWED_TO_FRAME)
			to_chat(user, span_notice("你开始将窗户从窗框上拧下来..."))
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				...
				to_chat(user, span_notice("你将窗户从窗框上松开了。"))
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("你开始将窗户拧到窗框上..."))
			if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
				state = WINDOW_SCREWED_TO_FRAME
				to_chat(user, span_notice("你将窗户固定到窗框上了。"))
		if(WINDOW_OUT_OF_FRAME)
			if(anchored)
				to_chat(user, span_notice("你开始将窗框从地板上拧下来..."))
				if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
					set_anchored(FALSE)
					to_chat(user, span_notice("你将窗框从地板上松开了。"))
			else
				to_chat(user, span_notice("你开始将窗框拧到地板上..."))
				if(tool.use_tool(src, user, decon_speed, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
					set_anchored(TRUE)
					to_chat(user, span_notice("你将窗框固定到地板上了。"))
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/window/crowbar_act(mob/living/user, obj/item/tool)
	...
	switch(state)
		if(WINDOW_IN_FRAME)
			to_chat(user, span_notice("你开始用撬棍将窗户从窗框中撬出..."))
		...
		if(RWINDOW_SECURE)
			if(tool.tool_behaviour == TOOL_WELDER)
				if(tool.tool_start_check(user, heat_required = HIGH_TEMPERATURE_REQUIRED))
					user.visible_message(span_notice("[user]将\the [tool]对准\the [src]上的安全螺丝..."),
						span_notice("你开始加热\the [src]上的安全螺丝..."))
					if(tool.use_tool(src, user, 15 SECONDS, volume = 100))
						to_chat(user, span_notice("安全螺丝已烧得白热，看起来可以拆除了。"))
						state = RWINDOW_BOLTS_HEATED
						addtimer(CALLBACK(src, PROC_REF(cool_bolts)), 30 SECONDS)
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("安全螺丝需要先加热！"))

		if(RWINDOW_BOLTS_HEATED)
			if(tool.tool_behaviour == TOOL_SCREWDRIVER)
				user.visible_message(span_notice("[user] 探入加热后的安全螺丝并开始拆卸它们..."),
										span_notice("你用力探入加热后的螺丝，它们开始转动了..."))
				if(tool.use_tool(src, user, 50, volume = 50))
					state = RWINDOW_BOLTS_OUT
					to_chat(user, span_notice("螺丝被取了出来，窗格边缘出现了一道缝隙。"))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("需要先卸下安全螺丝！"))

		if(RWINDOW_BOLTS_OUT)
			if(tool.tool_behaviour == TOOL_CROWBAR)
				user.visible_message(span_notice("[user] 将\the [tool] 楔入框架的缝隙并开始撬动..."),
										span_notice("你将\the [tool] 楔入框架的缝隙并开始撬动..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					state = RWINDOW_POPPED
					to_chat(user, span_notice("面板从框架中弹出，露出了几根看起来可以切割的细金属条。"))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("需要先把缝隙撬开！"))

		if(RWINDOW_POPPED)
			if(tool.tool_behaviour == TOOL_WIRECUTTER)
				user.visible_message(span_notice("[user] 开始切割\the [src] 上暴露的钢筋..."),
										span_notice("你开始切割\the [src] 上暴露的钢筋"))
				if(tool.use_tool(src, user, 20, volume = 50))
					state = RWINDOW_BARS_CUT
					to_chat(user, span_notice("面板掉落一旁，露出了框架螺栓。"))
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("需要先切割金属条！"))

		if(RWINDOW_BARS_CUT)
			if(tool.tool_behaviour == TOOL_WRENCH)
				user.visible_message(span_notice("[user] 开始将\the [src] 从框架上松开..."),
					span_notice("你开始从框架上松开螺栓..."))
				if(tool.use_tool(src, user, 40, volume = 50))
					to_chat(user, span_notice("你拧下了框架上的螺栓，窗户松动了。"))
					state = WINDOW_OUT_OF_FRAME
					set_anchored(FALSE)
			else if (tool.tool_behaviour)
				to_chat(user, span_warning("需要先松开螺栓！"))
...
/obj/structure/window/reinforced/crowbar_act(mob/living/user, obj/item/tool)
	...
	if(state != WINDOW_OUT_OF_FRAME)
		return FALSE
	to_chat(user, span_notice("你开始将窗户撬回框架中..."))
	if(tool.use_tool(src, user, 10 SECONDS, volume = 75, extra_checks = CALLBACK(src, PROC_REF(check_state_and_anchored), state, anchored)))
		state = RWINDOW_SECURE
		to_chat(user, span_notice("你将窗户撬回了框架中。"))
	return ITEM_INTERACT_SUCCESS

/obj/structure/window/proc/cool_bolts()
	if(state == RWINDOW_BOLTS_HEATED)
		state = RWINDOW_SECURE
		visible_message(span_notice("\the [src] 上的螺栓看起来已经冷却了..."))
...
/obj/structure/window/reinforced/examine(mob/user)
	...
	switch(state)
		if(RWINDOW_SECURE)
			. += span_notice("它被单向螺丝拧紧了，你需要<b>加热它们</b>才有可能拧出来。")
		if(RWINDOW_BOLTS_HEATED)
			. += span_notice("螺丝正发出白热的光芒，你现在很可能可以<b>拧下它们</b>了。")
		if(RWINDOW_BOLTS_OUT)
			. += span_notice("螺丝已被移除，露出了一个小缝隙，你可以把<b>撬具</b>伸进去。")
		if(RWINDOW_POPPED)
			. += span_notice("窗户的主板已经从框架中弹出，露出了一些看起来可以<b>切割</b>的栏杆。")
		if(RWINDOW_BARS_CUT)
			. += span_notice("主窗格可以轻松移开，露出一些固定框架的<b>螺栓</b>。")
...
/obj/structure/window/plasma
	name = "等离子窗"
	desc = "一种由等离子硅酸盐合金制成的窗户，它看起来非常难突破和烧毁。"
...
/obj/structure/window/reinforced/plasma
	name = "加固等离子窗"
	desc = "一种由棒材加固的等离子硅酸盐合金窗，它看起来难以打破，而且很可能几乎是防火的。"
...
/obj/structure/window/paperframe/attackby(obj/item/W, mob/living/user)
	...
	if(istype(W, /obj/item/paper) && atom_integrity < max_integrity)
		user.visible_message(span_notice("[user] 开始修补\the [src] 上的破洞。"))
		if(do_after(user, 2 SECONDS, target = src))
			atom_integrity = min(atom_integrity+4,max_integrity)
			qdel(W)
			user.visible_message(span_notice("[user] 修补了 \the [src] 上的一些破洞。"))
			if(atom_integrity == max_integrity)
				update_appearance()
			return
	..()
...
/obj/structure/window/bronze
	name = "黄铜窗"
	desc = "纸一样薄的半透明但加强过的黄铜窗格，不要紧，这只是弱一点的青铜！"
