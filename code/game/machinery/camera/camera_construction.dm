	...
	if(camera_construction_state != CAMERA_STATE_FINISHED || panel_open)
		if(attacking_item.tool_behaviour == TOOL_ANALYZER)
			if(!isXRay(TRUE)) //don't reveal it was already upgraded if was done via MALF AI Upgrade Camera Network ability
				...
				to_chat(user, span_notice("你将[attacking_item]安装到[name]的内部电路中。"))
				qdel(attacking_item)
			else
	...
	switch(camera_construction_state)
		...
		if(CAMERA_STATE_FINISHED)
			if(istype(attacking_item, /obj/item/modular_computer))
				...
				info = sanitize(info)
				to_chat(user, span_notice("你将\the [itemname]举到摄像头前..."))
				...
				for(var/mob/potential_viewer as anything in GLOB.player_list)
					if(isAI(potential_viewer))
						...
						if(user.name == "Unknown")
							to_chat(ai, "[span_name(user)]将<a href='byond://?_src_=usr;show_tablet_note=1;'>\a [itemname]</a>举到你的一台摄像头前...")
						else
							to_chat(ai, "<b><a href='byond://?src=[REF(ai)];track=[html_encode(user.name)]'>[user]</a></b>将<a href='byond://?_src_=usr;show_tablet_note=1;'>\a [itemname]</a>举到你的一台摄像头前...")
						continue
			...
			if(istype(attacking_item, /obj/item/paper))
				...
				to_chat(user, span_notice("你将\the [item_name]举到摄像头前..."))
				...
				for(var/mob/potential_viewer in GLOB.player_list)
					...
					if(isAI(potential_viewer))
						...
						if(user.name == "Unknown")
							to_chat(ai, "[span_name(user.name)]将<a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a>举到你的一台摄像头前...")
						else
							to_chat(ai, "<b><a href='byond://?src=[REF(ai)];track=[html_encode(user.name)]'>[user]</a></b>将<a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a>举到你的一台摄像头前...")
						continue
					...
					if (potential_viewer.client?.eye == src)
						...
						to_chat(potential_viewer, "[span_name(user)]将<a href='byond://?_src_=usr;show_paper_note=[REF(last_shown_paper)];'>\a [item_name]</a>举到你的摄像头前...")
				return
