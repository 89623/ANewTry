	...
	for(var/I in currentrun)
		...
		if(round_started && kick_inactive && !C.holder && C.is_afk(afk_period))
			...
			if (!isnewplayer(cmob) || !SSticker.queued_players.Find(cmob))
				...
				to_chat(C, span_userdanger("您已超过[DisplayTimeText(afk_period)]未活动，连接已被断开。</span><br><span class='danger'>您可以通过文件菜单中的按钮或<b><u><a href='byond://winset?command=.reconnect'>点击此处重新连接</a></u></b>来重新连接。"))
