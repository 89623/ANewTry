...
/obj/item/stack/medical/wound_recovery/try_heal(mob/living/patient, mob/living/user, healed_zone, silent, auto_change_zone, continuous)
	...
	if(HAS_TRAIT(woundies, TRAIT_WOUND_SCANNED))
		...
		if(user == patient)
			...
			user.visible_message(span_warning("[user] begins expertly treating the wounds on [patient]'s [limb.plaintext_zone] with [src]..."), span_warning("你开始快速处理[patient]的[limb.plaintext_zone]上的伤口，使用[src]，并牢记全息图像的指示..."))
	else
		user.visible_message(span_warning("[user] begins treating the wounds on [patient]'s [limb.plaintext_zone] with [src]..."), span_warning("你开始处理[user == patient ? "your" : "[patient]'s"] [limb.plaintext_zone]上的伤口，使用[src]..."))

	if(!do_after(user, treatment_delay, target = patient))
		return

	user.visible_message(span_green("[user] 将 [src] 敷在 [patient] 的 [limb.plaintext_zone] 上。"), span_green("你处理了 [user == patient ? "your" : "[patient]'s"] [limb.plaintext_zone] 上的伤口。"))
