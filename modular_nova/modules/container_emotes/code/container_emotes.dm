...
/mob/living/verb/container_emote()
	...
	if (isturf(src.loc))
		to_chat(src, span_danger("你不在任何东西里面！"))
		return
...
/datum/emote/container_emote/run_emote(mob/living/user, params, type_override = null, intentional = TRUE)
	...
	if(is_banned_from(user, "emote"))
		tgui_alert(user, "你无法发送表情（已被封禁）。")
		return FALSE
	else if(user.client?.prefs?.muted & MUTE_IC)
		tgui_alert(user, "你无法发送IC消息（已被禁言）。")
		return FALSE
	...
	if (!can_use)
		to_chat(user, span_danger("你不在任何东西里面！")) // If user is banned from chat, emotes, or the user is not within anything (ex. a locker) return.
		...
		container_emote = tgui_input_text(user, "你想要发送什么表情？", "容器表情", max_length = MAX_MESSAGE_LEN, multiline = TRUE)
		if(!container_emote)
			return FALSE
		var/list/choices = list("Visible","Audible")
		var/type = tgui_input_list(user, "这是一个可见的还是可听的表情？", "容器表情", choices, FALSE)
		switch(type)
			if("Visible")
			...
			if("Audible")
				...
				tgui_alert(user, "无法使用此表情，必须是可听或可见的。")
				return
	...
	if (length(locs_we_can_use) == 1)
		...
		picked_loc = tgui_input_list(user, "你希望你的表情从哪个容器发出？", "容器表情", locs_we_can_use, FALSE)
