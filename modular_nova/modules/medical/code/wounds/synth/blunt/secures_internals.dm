...
/datum/wound/blunt/robotic/secures_internals/proc/apply_gel(obj/item/stack/medical/bone_gel/gel, mob/user)
	...
	user.visible_message(span_danger("[user] 开始匆忙地将 [gel] 涂抹到 [victim] 的 [limb.plaintext_zone] 上..."), span_warning("你开始匆忙地将 [gel] 涂抹到 [user == victim ? "your" : "[victim]'s"] [limb.plaintext_zone] 上，无视它似乎对金属产生的酸性作用..."))
	...
	gel.use(1)
	if(user != victim)
		user.visible_message(span_notice("[user] 完成了将 [gel] 涂抹到 [victim] 的 [limb.plaintext_zone] 上，发出嘶嘶声！"), span_notice("你完成了将 [gel] 涂抹到 [victim] 的 [limb.plaintext_zone] 上！"), ignored_mobs=victim)
