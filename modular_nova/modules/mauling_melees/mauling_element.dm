...
/datum/element/mauling/proc/do_maul_target(obj/item/weapon, mob/living/carbon/target, mob/attacker)
	...
	attacker.visible_message(span_warning("[attacker]小心地将[weapon]瞄准[target]的[limb_target.plaintext_zone]！"), span_danger("你小心地将[weapon]瞄准[target]的[limb_target.plaintext_zone]！"))
	log_combat(attacker, target, "started aiming to maul", weapon)

	if(!do_after(attacker, swing_delay, target, interaction_key = weapon))
		return

	attacker.visible_message(span_warning("[attacker]挥舞[weapon]砍向[target]的[limb_target.plaintext_zone]！"), span_danger("You swing \the [weapon] at [target]'s [limb_target.plaintext_zone]!"))
