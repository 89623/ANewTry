...
/datum/wound/blunt/bone/severe/treat(obj/item/I, mob/user)
	...
	if(victim == user)
		...
		user.visible_message(span_danger("[user] 开始[scanned ? "expertly" : ""]用[victim]重新对齐[limb.plaintext_zone]的[I]。"), span_notice("你开始用[victim][limb.plaintext_zone]重新对齐[I]的[scanned ? ", keeping the holo-image's indications in mind" : ""]..."))
	...
	if(victim == user)
		...
		user.visible_message(span_danger("[user]以一声令人不安的<b>咔嚓</b>声重新对齐了[victim]的[limb.plaintext_zone]！"), span_nicegreen("你以一声令人不安的<b>咔嚓</b>声完成了对[victim]的[limb.plaintext_zone]的重新对齐！"), ignored_mobs=victim)
...
/datum/wound/blunt/bone/critical/treat(obj/item/I, mob/user)
	...
	if(victim == user)
		...
		user.visible_message(span_danger("[user]开始[scanned ? "expertly" : ""]用[victim]重新对齐[limb.plaintext_zone]的[I]。"), span_notice("你开始用[victim][limb.plaintext_zone]为[I]的[scanned ? ", keeping the holo-image's indications in mind" : ""]进行复位..."))
	...
	if(victim == user)
		...
		user.visible_message(span_danger("[user]以一声令人不安的<b>咔嚓</b>声完成了对[victim]的[limb.plaintext_zone]的复位！"), span_nicegreen("你以一声令人不安的<b>咔嚓</b>声完成了对[victim]的[limb.plaintext_zone]的复位！"), ignored_mobs=victim)
		to_chat(victim, span_userdanger("[user] resets your [limb.plaintext_zone] with a disturbing <b>crunch</b>!"))
