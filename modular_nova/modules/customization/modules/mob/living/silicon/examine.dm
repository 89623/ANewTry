...
/mob/living/silicon/proc/get_silicon_flavortext(mob/user)
	...
	flavor_text_link = span_notice("[silicon_preview_text]... <a href='byond://?src=[REF(src)];lookup_info=open_examine_panel'>仔细看看？</a>")

	if (flavor_text_link)
		. += flavor_text_link

	if (client?.prefs.read_preference(/datum/preference/text/character_ad))
		. += span_notice("他们在角色目录里有一则广告……<a href='byond://?src=[REF(src)];lookup_info=open_character_ad'>\[打开目录？\]</a>")
	...
	if (!CONFIG_GET(flag/disable_antag_opt_in_preferences))
		...
		if (!isnull(opt_in_status))
			...
			. += span_notice("反派参与状态：<b><font color='[GLOB.antag_opt_in_colors[stringified_optin]]'>[stringified_optin]</font></b>")

	if(temporary_flavor_text)
		if(length_char(temporary_flavor_text) <= 40)
			. += span_notice("<b>他们看起来与往常不同：</b> [temporary_flavor_text]")
		else
			. += span_notice("<b>他们看起来与往常不同：</b> [copytext_char(temporary_flavor_text, 1, 37)]... <a href='byond://?src=[REF(src)];temporary_flavor=1'>更多...</a>")
