...
/datum/component/interactable/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(params["item_slot"])
		...
		if(can_lewd_strip(source, target, item_index) && is_toy_compatible(new_item, item_index))
			...
			if(existing_item)
				source.visible_message(span_purple("[source.name] starts trying to remove something from [target.name]'s [item_index]."), span_purple("你开始从[existing_item.name]的[target.name]中取出[item_index]。"), span_purple("You hear someone trying to remove something from someone nearby."), vision_distance = SAMETILE_MESSAGE_RANGE, ignored_mobs = ignoring_mobs + list(target))
			else if (new_item)
			...
			if (source != target)
				target.show_message(span_warning("[source.name] 正试图 [existing_item ? "remove the [existing_item.name] [internal ? "in" : "on"]" : new_item ? "is trying to [insert_or_attach] the [new_item.name] [into_or_onto]" : span_alert("What the fuck, impossible condition? interaction_component.dm!")] 你的 [item_index]！"))
			if(do_after(
				source,
				5 SECONDS,
				target,
				...
				if(existing_item)
					...
					source.visible_message(span_purple("[source.name] [internal ? "inserts" : "attaches"]了[new_item.name] [into_or_onto] [target.name]的[item_index]。"), span_purple("你[insert_or_attach]了[new_item.name] [into_or_onto] [target.name]的[item_index]。"), span_purple("你听见附近有人[insert_or_attach]了什么东西[into_or_onto]某人。"), vision_distance = SAMETILE_MESSAGE_RANGE, ignored_mobs = ignoring_mobs)
