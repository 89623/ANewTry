...
/mob/dead/new_player/proc/get_title_html()
	...
	if(SSticker.current_state == GAME_STATE_STARTUP)
		...
		if(!SSticker || SSticker.current_state <= GAME_STATE_PREGAME)
			dat += {"<a id="ready" class="menu_button" href='byond://?src=[text_ref(src)];toggle_ready=1'>[ready == PLAYER_READY_TO_PLAY ? "<span class='checked'>☑</span> 已准备" : "<span class='unchecked'>☒</span> 未准备"]</a>"}
		else
			dat += {"
				<a class="menu_button" href='byond://?src=[text_ref(src)];late_join=1'>加入游戏</a>
				<a class="menu_button" href='byond://?src=[text_ref(src)];view_manifest=1'>船员名单</a>
				<a class="menu_button" href='byond://?src=[text_ref(src)];view_directory=1'>角色目录</a>
			"}

		dat += {"<a class="menu_button" href='byond://?src=[text_ref(src)];observe=1'>观察</a>"}

		dat += {"
			<hr>
			<a class="menu_button" href='byond://?src=[text_ref(src)];character_setup=1'>设置角色 (<span id="character_slot">[uppertext(client.prefs.read_preference(/datum/preference/name/real_name))]</span>)</a>
			<a class="menu_button" href='byond://?src=[text_ref(src)];game_options=1'>游戏选项</a>
			<a id="be_antag" class="menu_button" href='byond://?src=[text_ref(src)];toggle_antag=1'>[client.prefs.read_preference(/datum/preference/toggle/be_antag) ? "<span class='checked'>☑</span> 成为反派" : "<span class='unchecked'>☒</span> 不成为反派"]</a>
			<span class="menu_button info_display">延迟加入队列：[SStitle.get_latejoin_queue_count()]</span>
			<hr>
			<a class="menu_button" href='byond://?src=[text_ref(src)];server_swap=1'>切换服务器</a>
		"}
		...
		<script language="JavaScript">
			...
			var ready_marks = \[ "<span class='unchecked'>☒</span> 未准备", "<span class='checked'>☑</span> 已准备" \];
			...
			var antag_marks = \[ "<span class='unchecked'>☒</span> 不成为反派", "<span class='checked'>☑</span> 成为反派" \];
