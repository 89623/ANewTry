	...
	for(var/mob/living/silicon/ai/AI as anything in GLOB.ai_list)
		...
		if(AI.deployed_shell)
			if(!is_station_level(AI.deployed_shell.registered_z))
				continue
			to_chat(AI.deployed_shell, "<b><a href='byond://?src=[REF(AI)];track=[html_encode(user.name)]'>[user]</a></b>正在请求你打开[src][LINK_DENY][LINK_OPEN][LINK_BOLT][LINK_SHOCK]。")
			continue
		if(!is_station_level(AI.registered_z))
			continue
		to_chat(AI, "<b><a href='byond://?src=[REF(AI)];track=[html_encode(user.name)]'>[user]</a></b>正在请求你打开[src][LINK_DENY][LINK_OPEN][LINK_BOLT][LINK_SHOCK]。")
