	...
	switch(action)
		...
		if ("subtle_message")
			...
			if (sender == "Voice in head")
				...
				to_chat(selected_mob, "你听到耳机里传来一阵噼啪声，随后一个声音响起。  \"请稍候，等待来自 [sender == RADIO_CHANNEL_SYNDICATE ? "your benefactor" : "Central Command"] 的消息。  消息如下[sender == RADIO_CHANNEL_SYNDICATE ? ", agent." : ":"] <span class='bold'>[msg]。</span> 消息结束。\"")
